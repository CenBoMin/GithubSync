...
//阅读小说金币
if ($request && $request.method != `OPTIONS` && $request.url.match(/\/pendant\/lottery/)) {
  const spreadcoinurl = $request.url
  if (spreadcoinurl) $.setdata(spreadcoinurl, 'spreadcoinurl'+ $.idx)
  $.log(`获取spreadcoinurl请求🎉: 成功,spreadcoinurl: ${spreadcoinurl}`)
  const spreadcoinbody = $request.body
  if (spreadcoinbody) $.setdata(spreadcoinbody, 'spreadcoinbody'+ $.idx)
  $.log(`获取spreadcoinbody请求🎉: 成功,spreadcoinbody: ${spreadcoinbody}`)
  $.msg($.name + $.idx, "添加【极速-阅读小说金币】请求: 成功🎉")
  $.done()
}
//看视频1
...
if ($request && $request.method != `OPTIONS` && $request.url.match(/\/task\/reward/)) {
  const spsharebody = $request.body
  if (spsharebody) $.setdata(spsharebody, 'spsharebody'+ $.idx)
  $.log(`获取spsharebody请求🎉: 成功,spsharebody: ${spsharebody}`)
  $.msg($.name + $.idx, "添加【极速-邀请书友】请求: 成功🎉")
  $.done()
}
