...
if ($request && $request.method != `OPTIONS` && $request.url.match(/\/api\/getAdInfo/)) {
  ...
  if (videoinfourl) $.setdata(videoinfourl, 'videoinfourl'+ $.idx)
  $.log(`获取videoinfourl请求🎉: 成功,videoinfourl: ${videoinfourl}`)
  $.log(`添加【一般-视频判定Key】请求: 成功🎉`)
  //$.msg($.name + $.idx, "添加【一般-视频判定Key】请求: 成功🎉")
  $.done()
}
...
if ($request && $request.method != `OPTIONS` && $request.url.match(/\/task\/reward/)) {
  const sharebody = $request.body
  if (sharebody) $.setdata(sharebody, 'sharebody'+ $.idx)
  $.log(`获取sharebody请求🎉: 成功,sharebody: ${sharebody}`)
  $.msg($.name + $.idx, "添加【一般-邀请书友】请求: 成功🎉")
  $.done()
}
