const $ = new Env("WLKDAPP Cookie");
let wlkdapp = $.getjson('wlkdapp', []);
//++++++++++++++++++++++++++++++++++++
...
async function GetCookie() {
  ...
  if ($request.url.match(/\/login\/appOpen/)){
    ...
    if (userId) {
      ...
      $.msg($.name, '', `文旅看点[账号${no+1}] ${status?'新增':'更新'}Cookie数据成功！🎉`);
    } else {
      $.msg($.name, '', '文旅看点用户Cookie数据获取失败⚠️');
    }
