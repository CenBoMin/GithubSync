...
if ($.isNode()) {
  if (process.env.QQREAD_BODY && process.env.QQREAD_BODY.indexOf('#') > -1) {
  qqreadBD = process.env.QQREAD_BODY.split('#');
  console.log(`您选择的是用"#"隔开\n`)
  }
  else if (process.env.QQREAD_BODY && process.env.QQREAD_BODY.indexOf('\n') > -1) {
  qqreadBD = process.env.QQREAD_BODY.split('\n');
  ...
  else if (process.env.QQREAD_TIMEURL && process.env.QQREAD_TIMEURL.indexOf('\n') > -1) {
      qqreadtimeURL = process.env.QQREAD_TIMEURL.split('\n');
  } else {
      qqreadtimeURL = process.env.QQREAD_TIMEURL.split()
  };
  ...
  else if (process.env.QQREAD_TIMEHD && process.env.QQREAD_TIMEHD.indexOf('\n') > -1) {
      qqreadtimeHD = process.env.QQREAD_TIMEHD.split('\n');
  } else {
      qqreadtimeHD = process.env.QQREAD_TIMEHD.split()
  };
