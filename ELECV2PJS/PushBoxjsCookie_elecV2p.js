...
async function pushcookie(ckkey,ckvalue){return new Promise((resolve)=>{let url={url:`${v2purl}/webhook`,body:JSON.stringify({token:`${v2ptoken}`,type:'store',op:'put',key:ckkey,value:ckvalue,options:{type:'a',}}),headers:{'Content-Type':'application/json'},method:'put'};$.post(url,async(err,resp,data)=>{try{if(err){console.log("⛔️API查询请求失败❌ ‼️‼️");console.log(JSON.stringify(err));$.logErr(err);}else{data=JSON.parse(data);data.rescode==0?$.log(`✔️${data.message}`):$.log(`✖️${data.message}`)}}catch(e){$.logErr(e,resp);}finally{resolve();}});});}
