...
async function GetCookie() {
  ...
  if ($request.url.indexOf("receive-reward") > -1) {
    ...
    if (userId) {
      ...
      $.log(`获取成功🎉: sharecode: ${sharecode}`)
      $.msg($.name, '', `同程小猪猪[账号${no+1}] ${status?'新增':'更新'}Cookie数据成功！🎉`);
    } else {
      $.msg($.name, '', '同程小猪猪用户Cookie数据获取失败⚠️');
    }
